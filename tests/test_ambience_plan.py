...
def test_genitive_campfire_is_no_longer_missed():
    """Реальный найденный пробел 17.09: у «костёр» беглая гласная (костёр ->
    костра, костру, костром, костре), а словарь ловил только именительный/
    винительный. Самая естественная фраза «у костра» (родительный) не
    совпадала НИ С ЧЕМ в forge_fire и проваливалась в night весом 1 —
    то есть костёр в кадре не давал даже слабого сигнала."""
    assert ap.score_text("Они грелись у костра.").get("forge_fire", 0) >= 2
    assert ap.score_text("Путники устроили привал у большого костра.") \
             .get("forge_fire", 0) >= 2


def test_kostroma_is_a_known_accepted_tradeoff():
    """Основа «костр» (см. предыдущий тест) физически совпадает с городом
    Кострома. Для канала про европейское средневековье вероятность встретить
    это слово в сценарии практически нулевая — решение осознанное, а не
    незамеченный побочный эффект. Тест не проверяет отсутствие промаха (его
    нет смысла отрицать), а фиксирует, что промах ИЗВЕСТЕН: если он когда-то
    исчезнет (кто-то сузит основу), это не авария, которую надо чинить, а
    улучшение, которое можно заметить."""
    assert ap.score_text("Костромской купец вёл торговлю мехами.") \
             .get("forge_fire", 0) >= 2


def test_mountains_are_in_the_wind_vocabulary():
    """Реальный найденный пробел 17.09: запись «windy-mountain-plains» уже
    лежит в библиотеке wind_open, а слова «гора»/«горы» не было в словаре
    триггера вообще — библиотека умела, словарь не спрашивал."""
    for phrase in ("Холодные горы обдувал ледяной ветер.",
                   "Путники шли через горы под пронизывающим ветром."):
        assert ap.score_text(phrase).get("wind_open", 0) >= 4


def test_mountain_locative_case_is_deliberately_excluded():
    """«горе» (предложный падеж «в горе») — омограф со словом «горе» (грусть,
    беда). Тот же класс, что уже решён для «атлас» в stress_placement.py:
    честнее не ловить один падеж, чем один раз перепутать печаль с ландшафтом."""
    assert ap.score_text("Какое горе постигло семью.") == {}
