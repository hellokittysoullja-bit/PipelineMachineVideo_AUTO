...
class TestCultureQualifierStaysOutOfMuseum:
    ...
    def test_museum_gets_the_authored_query_stocks_get_the_qualified_one(self):
        ...
        # С 25.09 источники опрашиваются параллельно, и развилка «что
        # уходит в какой источник» живёт в _fetch_source фото-адаптера;
        # граница — следующий метод класса (по коду, а не по числу символов).
        start = src.index("    def _fetch_source(")
        block = src[start:src.index("\n    def ", start + 1)]
