...
class TestPhotoPath:
    ...
    def test_arbiter_cannot_run_at_all_when_the_director_is_off(self, tmp_path, monkeypatch):
        """Зафиксировано как факт текущего поведения, а не как «так и надо».

        Цикл подбора фото останавливается на ПЕРВОМ кандидате, прошедшем
        гейты, если Visual Director не просит больший пул. Пул из одного
        кандидата -> шорт-лист из одного -> `len(shortlist) >= 2` не
        выполняется -> VLM-арбитр не вызывается НИ РАЗУ, молча.

        То есть самый сильный судья в системе выключается побочным эффектом
        другого флага. В .env этого канала стоит `VISUAL_DIRECTOR_MODE=assist`,
        поэтому в проде арбитр работает — но связь неочевидна и нигде не
        проверялась. Тест держит её видимой: когда пул перестанет схлопываться
        (Блок 3), этот тест упадёт и потребует осознанного решения.
        """
        self._stub_photo(monkeypatch, tmp_path)
        called = []
        monkeypatch.setattr(shot_director, "arbitrate_hook_candidates",
        ...
        assert out is not None
        assert called == [], (
            "арбитр внезапно вызвался без Director — значит пул перестал "
            "схлопываться до одного кандидата; проверить и обновить ожидание")
        assert pipeline_smart.ARBITER_REJECTED_ALL == []
