...
class TestGoldenSetMetric:
    ...
    def test_leaks_closed_by_the_negative_veto_stay_closed(self, report, item_id, reason):
        """Два брака, которых до контрастивного вето не ловил НИ ОДИН гейт.

        Оба стояли в списке xfail выше как «известные утечки»; после вето
        они дали XPASS — и перенесены сюда, в обычные утверждения. Это и
        есть механика храповика: каждая закрытая утечка перестаёт быть
        пожеланием и становится требованием.
        """
        _, rows, _, _ = report
        row = next(r for r in rows if r["id"] == item_id)
