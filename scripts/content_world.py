...
def merge_content_world(base_profile, video_dir):
    ...
    # GENERIC_FALLBACKS КАНАЛА СНИМАЮТСЯ, а не переписываются (18.09).
    #
    # Найдено ИЗМЕРЕНИЕМ готового ролика, не чтением: в тестовом эпизоде про
    # ИСТОРИЮ КОФЕ слот #47 ушёл в сток с запросом «knight armor moody
    # light» и принёс кадр с релевантностью 0.124 при пороге 0.19 — то есть
    # объективный брак, который система сама признала не по теме и всё равно
    # поставила в ролик. Источник — пять средневековых запросов из
    # `generic_fallbacks` профиля КАНАЛА: они считаются при импорте модуля,
    # то есть ДО того, как вообще известна тема эпизода, и достаются любому
    # слоту, у которого не разрешился свой запрос.
    #
    # Это последний нишевый список, который авто-ниша не контролировала:
    # shot_domain, окно эпохи, музеи, блоклист и якоря эпохи она уже
    # переписывает по теме эпизода (см. выше), а этот шёл мимо — про него
    # не было ни строки в этом модуле.
    #
    # Почему СНЯТЬ, а не заменить своим списком: в pipeline_smart уже есть
    # `_generic_fallbacks_from_niche()`, которая строит ровно такие пары
    # «якорь эпохи + предметное существительное» из якорей ЭТОГО эпизода —
    # и она уже получает верные, переписанные выше якоря. Она просто
    # недостижима, пока канальный список непуст (`X or Y`). Снятие отдаёт
    # ей управление, и второй копии построения запросов не заводится.
    #
    # Порог тот же, что у остального мира эпизода: тема должна быть понята
    # уверенно. Ниже порога — канал остаётся как был, байт-в-байт.
    if confident_enough_to_override and merged.get("generic_fallbacks"):
        merged["generic_fallbacks"] = []
