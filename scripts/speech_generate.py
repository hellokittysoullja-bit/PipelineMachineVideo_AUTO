...
# ЧИСЛО + форма единицы. РЕАЛЬНЫЙ БАГ, который здесь был: подстановка
# всегда ставила ОДНУ И ТУ ЖЕ форму — родительный падеж множественного
# числа ("процентов"/"килограммов"). Для "50%" это верно, а для самых
# частых в этой нише случаев — нет: "1 кг" превращалось в "1 килограммов",
# "2 кг" — в "2 килограммов", "1,5 кг" — в "1,5 килограммов". И это не
# опечатка в отчёте: текст уходил В ОЗВУЧКУ и звучал вслух неграмотно,
# то есть баг платный (кредиты TTS) и слышимый зрителю. Для канала про вес
# меча "1,5 кг"/"2 кг" — буквально основной контент.
#
# Правило согласования (обычная школьная норма, без NLP): дробное число ->
# родительный падеж единственного числа ("1,5 килограмма"); целое,
# оканчивающееся на 11-14 -> "килограммов"; на 1 -> "килограмм"; на 2-4 ->
# "килограмма"; иначе -> "килограммов". Само число НЕ разворачивается в
# слова (это отдельная рискованная задача, см. комментарий выше) — меняется
# только форма существительного.
PRONUNCIATION_UNITS = [
    (re.compile(r'(\d+(?:[.,]\d+)?)\s*%'), ("процент", "процента", "процентов")),
    (re.compile(r'(\d+(?:[.,]\d+)?)\s*кг\b'), ("килограмм", "килограмма", "килограммов")),
    (re.compile(r'(\d+(?:[.,]\d+)?)\s*см\b'), ("сантиметр", "сантиметра", "сантиметров")),
    (re.compile(r'(\d+(?:[.,]\d+)?)\s*мм\b'), ("миллиметр", "миллиметра", "миллиметров")),
    (re.compile(r'(\d+(?:[.,]\d+)?)\s*км\b'), ("километр", "километра", "километров")),
]


def russian_unit_form(number_text, forms):
    """Форма единицы измерения, согласованная с числом.
    forms = (им.ед., род.ед., род.мн.) — например
    ("килограмм", "килограмма", "килограммов")."""
    one, few, many = forms
    if "," in number_text or "." in number_text:
        return few          # "1,5 килограмма", "0,7 килограмма"
...
def normalize_pronunciation(text):
    ...
    узкие и однозначные (см. PRONUNCIATION_UNITS) — ничего не разбирает
    семантически: единица измерения сразу после числа, форма согласуется с
    числом (см. russian_unit_form), само число не разворачивается в слова."""
    if not PRONUNCIATION_NORMALIZE_ENABLED:
        return text, []
    substitutions = []
    result = text
    for pattern, forms in PRONUNCIATION_UNITS:
        def _sub(m, forms=forms):
            number = m.group(1)
            after = f"{number} {russian_unit_form(number, forms)}"
