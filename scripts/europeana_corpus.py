...
(замер 15.09, живой API, ключ не нужен — см. `api_key()`):

    окно 1000-1600, только изображения, свободные права   209 142
    из них CC0                                              4 208
    из них Public Domain Mark                              33 817
    CC0 + PDM, то есть корпус этого модуля                 38 025
...
#: `crossbow` 5, `heraldry` 5 — при 38 262 записях в окне. Работают только
...
def year_clause(lo, hi):
    ...
    даёт 38 262. Ошибка была бы немой: пустая выдача у корпуса, который
