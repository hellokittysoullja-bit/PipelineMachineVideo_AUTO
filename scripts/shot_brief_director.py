...
3. СРАВНЕНИЯ НЕ ПОКАЗЫВАЙ. «Весил как холодильник», «поднимали краном»,
   «как перевёрнутая черепаха» — образы речи. Показывать надо то, О ЧЁМ
   речь, а не предмет сравнения.
...
7. НАСТРОЕНИЕ. Первой строкой ответа оцени главу по двум осям:
...
def rules_for(contract):
    ...
    if not contract:
        return RULES_CORE
    return RULES_CORE + (
        "\n8. МИР КАДРА — обязателен. " + contract +
