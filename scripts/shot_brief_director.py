...
def render_prompt(packet):
    ...
    if feature_flags.enabled("SHOT_BRIEF_DECOMPOSE_ACTION"):
        lines += [
            "",
            "ЕСЛИ ФРАЗА ОПИСЫВАЕТ МГНОВЕНИЕ ДЕЙСТВИЯ (падение, удар, отскок, "
            "точку столкновения) — такой кадр физически невозможно снять "
            "готовым стоковым кадром, его никогда не будет в источнике. "
            "Не пиши составную сцену целиком («рыцарь падает в грязь под "
            "тяжестью металла»). Назови её РАЗЛОЖЕННЫЙ, фотографируемый "
            "элемент: материал/текстуру места действия крупным планом "
            "(«грязь крупным планом» вместо «рыцарь падает в грязь»), "
            "состояние ДО или ПОСЛЕ момента, или обобщённое движение без "
            "полного сюжета («скачущая лошадь» вместо «конница атакует "
            "пехоту»). Деталь вместе с закадровым текстом достраивает "
            "смысл в голове зрителя — кадра, которого не существует, "
            "искать не нужно.",
        ]
