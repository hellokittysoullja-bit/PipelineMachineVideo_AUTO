...
def validate(card):
    ...
    if look is not None:
        if not isinstance(look, dict):
            bad.append("look должен быть объектом {style, renders_ok} или отсутствовать")
        else:
            if not isinstance(look.get("style"), str) or not look["style"].strip():
                bad.append("look.style должен быть непустой строкой")
            if not isinstance(look.get("renders_ok"), bool):
                bad.append("look.renders_ok должен быть true или false")
...
def is_historical(card):
    """Эпизод про прошлое: 3D, мультфильм и инфографика там — брак, и к
    запросам в сток приписывается якорь эпохи. У научного или абстрактного
    эпизода рендер бывает единственным изображением предмета, и бракуют его
    только по смыслу.

    `mixed` считается прошлым ТОЛЬКО при заданном окне эпохи — так регистр и
    определён (см. REGISTERS: «окно эпохи есть, но современные аналогии
    законны»). Живой случай 26.09: паспорт эп.95 (СДВГ, нейробиология)
    модель записала как `mixed` при `era=null`, и эпизод считался
    историческим. Отказ по CG отбросил 21 одобренный судьёй кадр (все 3D-мозги
    и модели дофамина в слотах 4–7), на экран встали игрушечные шарики, а ко
    всем запросам спецификации приписалось «neuroscience». Решение по
    структурному полю, а не по толкованию свободного слова: у эп.02 (mixed,
    1290–1480) поведение прежнее."""
    if not card:
        return False
    reg = card.get("register")
...
def renders_allowed(card):
    """Законны ли в этом фильме 3D-модели, анимация, рисунки, инфографика.

    Решает паспорт — поле look.renders_ok, которое модель пишет по сценарию
    и нише: один и тот же исторический материал в детском научпопе и в
    документальной реконструкции конкретной битвы требует разного. Жёсткое
    правило «история — значит без 3D» было хардкодом под нишу (живой
    случай эп.95, где научный ролик по ошибке считался историческим и
    потерял все 3D-мозги). Паспорт без этого поля (записан до 26.09) —
    прежнее правило, байт в байт."""
    look = (card or {}).get("look")
    if isinstance(look, dict) and isinstance(look.get("renders_ok"), bool):
        return look["renders_ok"]
    return not is_historical(card)
...
{{
  ...
  "look": {{"style": строка, "renders_ok": true | false}},
  "notes": строка
}}

Правила:
1. "era" — ГОДЫ ЦЕЛЫМИ ЧИСЛАМИ со знаком: -40000 значит 40000 год до н.э., 1415 значит 1415 год н.э. Окно берётся с запасом на весь материал эпизода, а не на одно событие. Если действие происходит сегодня или у эпизода нет привязки ко времени — null. Регистр "mixed" — только при заданном окне эпохи (прошлое с законными современными сравнениями); современная тема с наукой — "scientific" или "modern".
...
7. "look.style" — одной фразой по-английски: КАК этот фильм показывают зрителю — какой материал и какой характер кадров естественны для этой ниши и этой аудитории (например: кинематографичная реконструкция и подлинные старые изображения; бытовая современная съёмка вместе с научными снимками и 3D-моделями; яркие простые картинки для детей). Решай по сценарию и нише, не по шаблону.
8. "look.renders_ok" — может ли в ЭТОМ фильме кадром быть компьютерная графика: 3D-модель, анимация, мультяшная или игровая картинка вместо съёмки реального мира и подлинных изображений. Реши по аудитории и цели фильма. true — когда графика помогает понять или нравится этому зрителю: объяснение того, что нельзя снять камерой (строение мозга, молекула, космос), детский фильм. false — когда фильм о подлинных людях, предметах и событиях для взрослого зрителя и компьютерная картинка рядом с настоящими предметами, живой съёмкой и старыми изображениями выглядит подделкой.
9. Ничего не выдумывай про мир, чего нет в тексте. Список пустой — так и оставь пустым.
...
def world_digest(card):
    ...
    if not card:
        return ""
    import hashlib
    fields = {k: card.get(k) for k in WORLD_FIELDS}
    if card.get("look") is not None:
        # Облик фильма решает отказ по 3D и задание планировщика; у паспортов
        # без него отпечаток прежний — кэш отбора старых эпизодов не сгорает.
        fields["look"] = card.get("look")
    body = json.dumps(fields, sort_keys=True, ensure_ascii=False)
