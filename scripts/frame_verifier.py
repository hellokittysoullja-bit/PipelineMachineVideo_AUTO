...
#
# 3 (19.09, тот же день) — ПРЯМАЯ ЖАЛОБА ВЛАДЕЛЬЦА («система стала тупее
# после всех улучшений, раньше просто «кинжал» находил кинжал») привела к
# находке, которая версией 2 и была создана: INTENT_CLAUSE подавала бриф
# автора как ЧТО ИМЕННО НУЖНО ПОКАЗАТЬ буквально — и модель стала требовать
# СОВПАДЕНИЯ ВСЕХ деталей брифа, включая второстепенные (фон, материал,
# поза), не только предмета/действия. Живой прогон эпизода после версии 2:
# из 20+ РЕАЛЬНЫХ кинжалов в пуле НИ ОДИН не принят («seen: кинжал в
# ножнах на столе», «missing: рука в латной перчатке»; «seen: изогнутый
# кинжал в ножнах», «missing: круглая гарда, без ножен»; и т.д. — причина
# отказа всегда несущественная деталь сцены, никогда не сам предмет).
# Контрольный вызов на настоящем музейном кинжале Мет (plain white
# background, met:32684 — тот самый, что в этом файле уже приводился как
# образцовая находка): «seen: medieval dagger on white background»,
# «missing: dark wood background» — модель САМА подтвердила, что видит
# кинжал, и всё равно отклонила его за то, что фон не деревянный. Бриф был
# написан для CLIP-поиска (там больше деталей — точнее эмбеддинг), а не для
# этого гейта — вопрос его не различал. INTENT_CLAUSE переписана: остаётся
# только предмет/действие из брифа, второстепенные детали сцены явно
# названы необязательными для вердикта.
PROMPT_VERSION = 3
...
#
# ВТОРОЕ ПРЕДЛОЖЕНИЕ — РЕАЛЬНЫЙ, ЖИВЫМ ПРОГОНОМ НАЙДЕННЫЙ ФИКС (19.09,
# PROMPT_VERSION 3, см. её докстринг). Без него формулировка «что именно
# нужно показать» модель читала буквально и требовала совпадения ВСЕХ
# деталей брифа — бриф пишется для CLIP-поиска, где больше деталей даёт
# точнее эмбеддинг (ЧАСТЬ 13, Шаг 3), и по конструкции содержит детали
# сцены (фон, материал, поза), которых у реального кандидата может не
# быть, хотя предмет — тот самый. Живой пример: настоящий музейный кинжал
# (met:32684, белый фон) отклонён с «missing: dark wood background» —
# фон из брифа, не сам кинжал. Второе предложение явно снимает эти детали
# с проверки, оставляя обязательным только предмет/действие.
INTENT_CLAUSE = (
    "Уточнение автора сценария, что именно нужно показать на этом кадре "
    "(разрешает местоимения и абстракции во фразе выше): «{brief}». "
    "Обязателен для вердикта только ПРЕДМЕТ ИЛИ ДЕЙСТВИЕ, которое названо в "
    "этом уточнении — остальные детали (фон, материал, освещение, поза, "
    "точная форма второстепенной части предмета) автор писал для ПОИСКА "
    "картинки, а не как обязательный список для проверки; их отсутствие на "
    "кадре само по себе не повод для «нет».\n\n"
)
