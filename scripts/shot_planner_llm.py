...
# «Думающая» модель вслух рассуждает нумерованными пунктами — реальный,
# уже случившийся отказ: Qwen3.6-35B-A3B дала семнадцать «заявок» вида
# «1. Analyze User Input», пересказ правил промпта, а не ответ (см.
# CLAUDE.md, раздел про пофразовый автомат). Требование разделителя `|`
# в `_ROW_RE` сузило риск, но не закрыло его: рассуждение тоже может
# случайно содержать `|`. Блок `<think>...</think>` снимается целиком —
# для модели, которая не думает вслух (дефолт этого канала), это
# безопасный no-op: тега там нет вообще. Незакрытый `<think>` (модель
# упёрлась в потолок токенов посреди рассуждения и до ответа не дошла,
# тот самый случай 35B) — снимается ДО КОНЦА строки: после него нет
# ничего, что можно было бы честно спасти.
_THINK_RE = re.compile(r"<think>.*?</think>", re.S | re.I)
_UNCLOSED_THINK_RE = re.compile(r"<think>.*", re.S | re.I)


def _clean_stream(text):
    """Снять оформление потока и рассуждение модели, не трогая сам ответ."""
