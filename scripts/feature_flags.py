...
FLAGS = {f.name: f for f in (
    ...
    # чтобы в любой сессии было автоматически"). Прежний дефолт 0 не имел
    # под собой найденной причины безопасности/денег/лицензии — только
    # историческое "опционально" с момента, когда источник впервые
    # появился. По дисциплине лицензий Openverse в ТОМ ЖЕ классе, что и
    # музеи (MUSEUM_SOURCES_ENABLED, дефолт 1): только license=cc0,
    # fail-closed повторная проверка на каждом результате, белый список
    # институциональных source (см. OPENVERSE_TRUSTED_SOURCES) — то же
    # обоснование, что уже дало музеям дефолт 1, здесь просто не было
    # применено раньше. Ключ не нужен, платных вызовов нет.
