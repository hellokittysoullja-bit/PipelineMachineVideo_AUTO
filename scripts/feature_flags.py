...
FLAGS = {f.name: f for f in (
    ...
    Flag("SHOT_PLANNER_LLM", "1",
         summary="Локальный режиссёр на CPU: фраза диктора -> описание кадра "
                 "(заполняет тот же shot_brief, что пишет автор). "
                 "Дефолт 1 безопасен вдвойне: без LLAMA_CLI_BIN/LLAMA_MODEL_GGUF "
                 "в окружении runtime_ready()=False и это точный no-op; там, где "
                 "модель есть, plan_episode() трогает только юниты БЕЗ авторского "
                 "shot_brief, то есть на уже написанных вручную эпизодах (в т.ч. "
                 "02_ne-mechom, 142/142 брифов) байт-в-байт ничего не меняет."),
    Flag("SHELF_INDEX", "1",
