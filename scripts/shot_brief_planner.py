...
СТАТУС: ИССЛЕДОВАТЕЛЬСКИЙ ПРОТОТИП, В РЕНДЕР НЕ ПОДКЛЮЧЁН И НЕ БУДЕТ
(помечено 16.09). Задачу решает `scripts/shot_brief_director.py`: он
спрашивает ГЛАВУ с контекстом, а не фразу, и замерен на срезе 116 юнитов
против эталона (69 попаданий у лучшей локальной модели против 58 у
запроса секции — `docs/quality/SHOT_BRIEF_DIRECTOR.md`). Этот модуль
замера против эталона не проходил НИ РАЗУ.

Файл оставлен НЕ по недосмотру: разбор внизу (режимы отказа
contrastive-моделей, буквальное против фигурального, почему короткий
запрос ломается) — это материал, на котором построен работающий модуль,
и он не дублируется нигде. Его функции стоят в `ALLOWED_UNREACHABLE`
теста `tests/test_no_dead_layers.py` С ПРИЧИНОЙ — то есть числятся
инвентарём, а не тихо спят. Не подключать, не развивать: две реализации
одной задачи рядом — ровно тот класс, которым этот репозиторий уже горел
(два словаря тегов стоили эпизоду PHRASE LOCK).
