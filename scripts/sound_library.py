...
def import_file(src, dst, kind, dur):
    ...
    body = t1 - t0
    # Шов НЕ выключается на короткой записи — он масштабируется. `_seamless_loop`
    # уже делает `xf = min(xf, body / 4.0)` сама и отказывается только ниже 0.1с,
    # то есть то же правило жило здесь ВТОРОЙ копией, и копия была грубее:
    # `body > 12` при xf=3.0 обнуляло шов целиком.
    #
    # До снижения нижней границы приёма 45 -> 15 (решение владельца 17.09) эта
    # ветка была практически недостижима — 45-секундная запись даёт body far
    # больше 12. После снижения она стала рабочей: запись 15.2с с авторским
    # затуханием 3.5с даёт body 11.7 -> шов НЕ собирался, и петля щёлкала на
    # КАЖДОМ обороте, молча (см. историю «бесшовная петля не собиралась ни разу»).
    # Для любой записи с body > 12 значение прежнее (min(3.0, body/4) == 3.0),
    # то есть всё, что уже лежит в библиотеке, импортируется байт-в-байт так же.
    xf = AMBIENCE_LOOP_XFADE_SEC
