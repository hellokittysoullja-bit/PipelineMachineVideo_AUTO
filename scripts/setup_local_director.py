...
MODELS = {
    "30b": {
        ...
        # score 69 снят пакетом версии 4 и на текущем задании НЕ
        # подтверждён — сравнивать его с 63 у маленькой нельзя (см. pick).
        "gb": 12.4, "need_ram_gb": 15, "score": 69, "minutes": 30,
        "sec_per_chapter": 180, "score_stale": True,
    },
    "4b": {
        ...
        "sec_per_chapter": 35, "default": True,
    },
}
...
def pick(ram_gb, forced=None):
    """По умолчанию — МАЛЕНЬКАЯ, независимо от объёма памяти (решение
    владельца 16.09, подкреплённое замером времени).

    Раньше на машине с 15+ ГБ бралась крупная: её 69 попаданий против 63
    у маленькой. Это число ОКАЗАЛОСЬ НЕСРАВНИМЫМ — оно снято пакетом
    версии 4, а маленькая мерилась на другой; та же ловушка, что уже
    поймана на квантовке. То есть преимущество крупной сегодня НЕ
    УСТАНОВЛЕНО, а её цена установлена и велика:

      маленькая (2.3 ГБ)   ~35 с на главу    ~8 мин на эпизод
      крупная   (12.4 ГБ)  ~180 с на главу   ~40 мин на эпизод, 11 ГБ ОЗУ

    Впятеро дольше и почти вся память машины — за выигрыш, которого никто
    не показал на текущем задании. Крупная остаётся доступной явным
    `--model 30b` для того, кто захочет её проверить сам.
    """
    if forced:
        return forced
    return "4b"
