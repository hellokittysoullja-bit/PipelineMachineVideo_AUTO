...
СЕЙЧАС НЕ ПОДКЛЮЧЕНО (решение владельца 23.09): все цели подбора должны
выполняться на найденных кадрах, генерация — будущая добавка. Модуль и
его тесты держат задел в рабочем состоянии, но ни один путь отбора его не
вызывает (tests/test_no_dead_layers.py знает об этом поимённо).

КАК ВСТАНЕТ, когда будет подключено. Сгенерированный кадр — не замена
поиску и не отдельная ветка выбора, а КАНДИДАТ того же пула: судья
сравнивает его с найденными на одной сетке. Поэтому неудачная генерация
(не та эпоха, лишняя рука, современная вещь, которой требует фраза)
просто проигрывает — выбор лучшего из пула по построению не ухудшает слот.
