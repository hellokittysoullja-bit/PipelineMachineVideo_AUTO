...
# граница reveal_hold (1.4с) — всё ещё в пределах "1.5-3с тишины перед
# разоблачением" из продакшн-разбора (см. CLIMAX_DIP_* в pipeline_smart.py),
# но без выхода за словарь допустимых тегов — сам [pause]=0.8 остаётся
# ЗАЯВЛЕННОЙ длиной тега, а 0.9-1.4 здесь — это то, чего Cadence Validator
# позволяет ДОСТИЧЬ монтажной обрезкой (fix_pauses.py), не то, что просит
# от диктора текстовым тегом сверх словаря.
RHETORICAL_RANGES = {
    "reveal_hold":    (0.90, 1.40),
