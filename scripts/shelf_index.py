...
def index_present():
    """Есть ли индекс НА ДИСКЕ — без загрузки матрицы и без ML-стека.

    Заведена по найденному дефекту (16.09): единственным способом спросить
    «спрашивают ли полку» была available(), а она зовёт load(), то есть
    читает ВСЮ матрицу векторов через np.fromfile (136 МБ на индексе из
    30 957 предметов) и через stack_signature() делает `import torch,
    transformers`. Для строки ключа кэша кандидата это абсурдная цена:
    ключ обязан быть чистой строковой операцией.

    Хуже цены — недетерминизм: ключ зависел от того, УДАЛОСЬ ли загрузить
    индекс. Сбой numpy, полузаписанный индекс во время сборки, гонка — и
    ключ всего прогона переключался, а файлы кэша, записанные «с полкой»,
    становились недостижимы. Наличие файла стабильнее успешного разбора.

    Честный предел: файл есть, но не читается — ключ назовёт вопрос к
    полке, которого не случилось. Цена этого ровно одна лишняя ветка
    ключа, а не 136 МБ и не потеря кэша.
    """
    try:
        return (os.path.getsize(ITEMS_PATH) > 0
                and os.path.getsize(VECTORS_PATH) > 0)
    except OSError:
        return False
