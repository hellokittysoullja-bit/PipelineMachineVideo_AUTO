...
def name_agreement(brief, limit=5):
    ...
    Замер на 12 брифах: у всех шести, чей предмет на полке есть, совпадение
    не меньше 1 из 5; у всех шести отсутствующих и заведомо чужих — ровно
    0 из 5. Перекрытия нет.

    ЧЕСТНЫЕ ПРЕДЕЛЫ, названные прямо. Двенадцать случаев и разметка автора
    правки — это проверка на вменяемость, а не приёмка, поэтому величина
    ничего НЕ блокирует и ни одного кандидата не отклоняет: она печатается
    человеку. И отвечает она на вопрос «полка ответила о том, что просили»,
    а не «кадр хороший» — второе решают гейты и глаз на Шаге 7.5."""
