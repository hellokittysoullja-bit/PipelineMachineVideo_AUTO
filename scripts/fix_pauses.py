...
def main():
    ...
    if not probe:
        # ffprobe есть, но ничего не сказал (ненулевой код возврата, файл
        # без аудиодорожки, чужой контейнер) — откат на дефолты тот же, а
        # молчать о нём нельзя: «замер не состоялся» и «замер дал ровно
        # эти значения» — разные утверждения, и по логу прогона они обязаны
        # различаться.
        print(f"  Формат исходника не измерен (ffprobe не дал вывода) — "
              f"тишина вставляется как {SILENCE_RATE} Гц/{SILENCE_LAYOUT}")
    if probe:
        bits = (probe.split(",") + ["", ""])[:2]
