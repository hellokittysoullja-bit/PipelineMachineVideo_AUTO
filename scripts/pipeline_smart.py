...
# файл ~2-3ГБ на 17-минутный эпизод, ожидался 400-700МБ). Измерено, не
# угадано: реальный клип этого канала (grain+vignette+halation, 8.25с,
# 1080p) при FINAL_PASS_CRF=16 даёт ~23.6 Мбит/с видео — на длительность
# эпизода это математически ровно ~3.1ГБ, объясняет жалобу полностью,
# это не баг, а прямое следствие CRF16 на кадре с синтетическим зерном
# на КАЖДОМ кадре (GRAIN_ENABLED) — шум дорог для сжатия по построению.
#   -tune grain ПРОВЕРЕН И ОТКЛОНЁН: на том же тестовом клипе дал файл
#   БОЛЬШЕ (51.6->60.6МБ, +17%) при той же PSNR против lossless-эталона
#   (26.88->26.85дБ) — гипотеза "та же картинка дешевле" была неверна,
#   tune=grain у x264 сохраняет зерно ТОЧНЕЕ ценой бит, не экономит их.
#   Битрейт аудио (192k AAC) и профиль кодека (auto High для 8-bit) —
#   не источник, проверены отдельно, вклад <1% от видеопотока.
#   CRF-лестница на том же клипе: CRF16=49МБ CRF19=40МБ CRF22=19МБ
#   CRF25=9МБ — чтобы попасть в 400-700МБ на полный эпизод, нужен CRF
#   в районе 23-25, это РЕАЛЬНЫЙ размен качества (не проверено на тёмных
#   градиентах/углах виньетки, где стоит deband — самый рискованный
#   случай для бандинга; PSNR на зерне ненадёжна как метрика видимости
#   потерь, см. выше). Сознательно НЕ применено — это творческое
#   решение (как и BODY.vign, см. коммит 2ebe746), а не техническая
#   правка, и должно быть решением пользователя, не автономным.
