...
# ACTION_STEMS выше — про БОЙ, потому что канал сегодня про бой; но
# "летит"/"раскрывается"/"зависает"/"опускает" — движение, а не бой, и
# ни на одной боевой основе не совпадают. До этой правки такая фраза
# уходила на голый хэш h_text % 2 — то есть "аппарат летит к планете" и
# "раскрывается парашют" получали фото/видео монеткой, ровно тот же
# класс дефекта, который ACTION_STEMS уже закрыл для боя (см. её
# докстринг: "в 97% случаев решение принимал h_text % 2").
#
# Основы — ОБЩИЕ, не про конкретную нишу (в отличие от боевых): полёт,
# раскрытие/разворачивание механизма, зависание, спуск/подъём, вращение,
# парение, плавание, дрейф. Каждая основа выбрана ДЛИННЕЕ, чем нужно для
# омонима-ловушки, а не короче + список исключений — это конструктивно
# исключает трижды измеренный класс дефекта ("зал" в "ЗАЛП", "боя" в
# "боятся"): длина строки уже не даёт совпасть.
#
# Проверено на реальном корпусе пяти разных ниш этого репозитория
# (videos/*/script.txt — Средневековье, кофе, глубоководье, Марс,
# смешанный тест; 2409 уникальных слов) — совпадения только настоящие
# ("летит", "летят", "полетят", "полёта", "раскрывается", "зависает",
# "опускает", "поднимали", "поднялся", "плавания", "плавательного",
# "спускают", "спустились"), ловушек ноль. Отдельно проверено против
# известных русских омонимов, которые короче основы бы поймали: "зависИТ"/
# "зависимость" (не совпадает с "зависа", т.к. 6-я буква "и", не "а"),
# "летОМ"/"летний"/"летопись"/"летальный" (не совпадают с "летит"/"летят"/
# "полёт" — разные буквы после общего "лет"), "вертолёт" (не совпадает с
# "вертит"/"вертящ" — 5-я буква "о", не "и"), "качество" (не совпадает с
# "покачива"), "плавка"/"плавление" (не совпадает с 5-буквенной "плава" —
# 5-я буква "к"/"л", не "а"), "опустошил"/"запустение" (не совпадают с
# "опустил"/8 букв и с "запуска" — разный корень, "опуст-о-шил" против
# "опуст-и-л"). Всё это — не гипотезы, а реальный запуск теста.
#
# Сознательно НЕ включено: "сниж-"/"снижа-" ("снижается") — в explainer-
# контенте это чаще метафора (снижение спроса/показателей), чем реальный
# физический спуск, тот же принцип, что уже применён к "падает"/"рухнул"
# в MOTION_VIDEO_QUALIFIERS ниже: честнее не поймать, чем поймать вредно.
MOTION_STEMS = (
    "летит", "летят", "летел", "летели", "летящ", "полет", "полёт",
    "взлет", "пролета",
    "раскры", "разверты", "разворачива",
    "зависа", "зависш",
    "опуска", "опустил",
    "поднима", "поднял",
    "вертит", "вертящ", "враща",
    ...
    "плава", "плыв",
    "спуска", "спустил",
    "дрейф", "вибрир", "колебл", "покачива",
)


def has_motion_word(text):
    """Описывает ли блок физическое движение объекта (не бой) — второй,
    нишево-независимый критерий заказа ВИДЕО. Тот же стоп-список омонимов
    ACTION_STEM_EXCLUDE не нужен: основы MOTION_STEMS выверены длиной, см.
    докстринг выше — но общая чистка мусора вокруг слова (пунктуация,
    регистр) та же, что у has_action_word()."""
    for w in text.split():
        w = re.sub(r'[^\w]', '', w.lower())
        if not w:
            continue
        if any(w.startswith(s) for s in MOTION_STEMS):
            return True
    return False
...
def action_video_qualifier(text):
    ...
    Проверяет ОБЕ таблицы — боевую (ACTION_VIDEO_QUALIFIERS) и общую
    (MOTION_VIDEO_QUALIFIERS, см. ниже) — одной функцией, а не двумя：
    вызывающий код (act_qual в main()) ждёт ровно одно значение на слот,
    и разводить его по двум функциям означало бы либо звать обе и решать
    порядок снаружи, либо тихо забыть про вторую на одном из вызовов —
    тот же класс асимметрии, что уже стоил репозиторию половины эпизода
    четыре раза (см. CLAUDE.md, filter_alt_blocklist/director_score_fn/
    бриф стокам/чередование источников)."""
    if not text:
        return None
    words = [re.sub(r'[^\w]', '', w.lower()) for w in text.split()]
    action_words = [w for w in words if w and w not in ACTION_STEM_EXCLUDE]
    for stems, qualifier in ACTION_VIDEO_QUALIFIERS:
        if any(w.startswith(s) for w in action_words for s in stems):
            return qualifier
    motion_words = [w for w in words if w]
    for stems, qualifier in MOTION_VIDEO_QUALIFIERS:
        if any(w.startswith(s) for w in motion_words for s in stems):
            return qualifier
    return None


# Английские уточнения к видео-запросу для MOTION_STEMS — та же дисциплина,
# что у ACTION_VIDEO_QUALIFIERS: формулировки ПРОВЕРЕНЫ живым Pexels API
# (не подобраны по интуиции), часть отклонена по факту выдачи.
#
#   'spacecraft flying' -> 6/6 настоящих космических кораблей.
#   'parachute deploying' -> 5/5 настоящих раскрытых парашютов (параплан/
#     парашютисты) — та самая фраза владельца ("раскрывается парашют").
#   'hovering drone' -> 6/6 дронов в воздухе.
#   'rotating slowly' / 'rotating' -> шестерни, абстрактное вращение,
#     ноль мусора на обоих формулировках.
#   'rising' -> небоскрёбы, рассвет, воздушный шар — по теме, без мусора.
#   'descending' -> честно НЕ идеально (эскалаторы, лестницы — люди, не
#     механизм/аппарат), но не мусор и не хуже, чем без уточнения совсем;
#     реальный вызов всегда ДОБАВЛЯЕТ уточнение к уже специфичному запросу
#     слота (apply_action_qualifier достраивает, не заменяет), поэтому
#     чистая проверка голым словом — нижняя граница, не итог.
#
# Отклонено по факту живой выдачи, как раньше отклонили "swinging":
#   'unfolding' (голое слово) -> лепестки цветка, салфетка, конверт — про
#     бумагу и ткань, ноль общего со "раскрывается парашют/механизм".
#     "раскры-"/"разверты-"/"разворачива-" уточняются словом 'deploying'.
#   'floating' (голое слово) -> сплошь люди, плавающие в бассейне — не
#     плывущий/дрейфующий предмет. "плава-"/"плыв-" остаются без
#     уточнения: основа всё равно форсирует видео (MOTION_STEMS), просто
#     без уточнения запроса — честнее, чем уточнять во вред.
#   Дрейф/вибрация/покачивание не проверены живьём вовсе — без уточнения,
#     по тому же принципу "не уверен — не уточняй".
MOTION_VIDEO_QUALIFIERS = (
    (("летит", "летят", "летел", "летели", "летящ", "полет", "полёт",
      "взлет", "пролета"), "spacecraft flying"),
    (("раскры", "разверты", "разворачива"), "parachute deploying"),
    (("зависа", "зависш", "паря", "парящ", "парит"), "hovering drone"),
    (("опуска", "опустил", "спуска", "спустил"), "descending"),
    (("поднима", "поднял"), "rising"),
    (("вертит", "вертящ", "враща"), "rotating slowly"),
)
...
# и до половины.
#
# ENV-ПЕРЕОПРЕДЕЛЕНИЕ (19.09, прямой запрос владельца «0 брака, а не
# случайный кадр»). На КОРОТКОМ эпизоде формула `int(n_slots * 0.08)`
# округляется вниз до нуля/единицы: живой замер на 9-слотовом тестовом
# эпизоде — бюджет 1 карточка при 6+ слотах, где гейт САМ подтвердил брак
# (FRAME_VERIFIER_GAVE_UP). 8% — разумный потолок на ПОЛНОМ эпизоде (сотни
# слотов, карточки не должны стать половиной ролика), но на коротком/
# тонком по корпусу эпизоде та же формула глушит собственную цель бюджета —
# показывает подтверждённый брак вместо честной карточки просто потому,
# что процент округлился в ноль. Дефолт БАЙТ-В-БАЙТ прежний (0.08/3) —
# переопределение только по явному запросу для конкретного прогона.
FALLBACK_CARD_MAX_SHARE = float(os.environ.get(
    "FALLBACK_CARD_MAX_SHARE", "0.08"))   # не больше N% слотов эпизода
...
FALLBACK_CARD_MIN_GAP = int(os.environ.get(
    "FALLBACK_CARD_MIN_GAP", "3"))   # минимум столько слотов между карточками
...
_CONTENT_ALT_BLOCKLIST_DEFAULT = (
    ...
    # Найдено вживую 19.09 на живом медиевал-тесте, разбором жалобы
    # владельца «решал исход поединка» -> современный человек в траве:
    # после фикса зрячего гейта на это место пришёл настоящий кинжал, но
    # им оказался ближневосточный/османский джамбия (изогнутый клинок,
    # латунные ножны с гравировкой) — Pixabay id 1341217, теги буквально
    # "dagger, crooked dagger, knife, ORIENTAL, black". Ни VISUAL_DOMAIN_
    # GUARDS (калиброван на паре европейский longsword/катана-цзянь, не на
    # эту культуру), ни NEGATIVE_VETO (нет ловушки под эту форму клинка) её
    # не поймали — только текстовый тег. relevance 0.31, оба существующих
    # гварда молчат (проверено живьём), поэтому без слова в блоклисте кадр
    # проходит все гейты чистым.
    "oriental",
...
#
# 1 -> 2 (17.09): чередование ПО ИСТОЧНИКАМ перенесено и на pexels_video() —
# до этого там было "весь Pexels, потом весь Pixabay" внутри одного запроса
# (то же самое, что у фото чинили 13.09, но никогда не портировали на видео).
# Живой прогон с замоканными источниками (40 Pexels + 40 Pixabay, один
# запрос, VIDEO_RELEVANCE_MAX_TRIES=3): первые 3 "offered" — ВСЕ Pexels,
# ни одного Pixabay. Второй видео-корпус, заведённый именно против брака
# 63% у видео-слотов, для однозапросных слотов (самый частый случай) не
# участвовал в реальном отборе НИКОГДА — не тратил трафик впустую, но и не
# давал ролику ничего, ровно тот класс, которым этот репозиторий уже горел
# пять раз. Подъём версии обязателен — без него исправление не дойдёт до
# экрана на прогретом temp_smart/pexels_video_cache/.
POOL_SOURCE_INTERLEAVE_VERSION = 2
...
def pexels_photo(query, index, used_ids=None, used_hashes=None, recent_sizes=None, target_luma=None,
    ...
    try:
        ...
        if used_hashes is None:
            ...
            # файла с его же sidecar на живом прогоне — meta.json называл
            # кандидата "gauntlets holding a sword", а сам jpg на диске
            # оказался рукописной миниатюрой битвы). Причина: цикл резкости
            # выше пересчитывает `pick = winner["p"]` на КАЖДОЙ итерации (см.
            # "while True" выше), поэтому там `pick` и `winner` не расходятся
            # никогда. Этот цикл — про зрячий гейт — переставляет `winner` и
            # перекачивает `cf` под НОВОГО победителя (строки выше), но ни
            # разу не трогает `pick`. Если гейт хоть раз отклонил кандидата,
            # `pick` замирает на ПЕРВОМ (уже отвергнутом) кандидате, а `cf` на
            # диске и `winner` в памяти — на последнем. Всё, что ниже читает
            # `pick` (dedup `used_ids`, провенанс/лицензия в
            # source_license_manifest.jsonl, `pexels_id`/`candidate_text` в
            # sidecar, кредит источника в source_contribution.json), после
            # этого называет ДРУГОГО кандидата, чем тот, что реально попал в
            # кадр — не косметика: дедуп не видит показанный кадр вообще (он
            # может выиграть слот второй раз как "другой" кандидат), а
            # юридический манифест называет не тот источник/лицензию.
            # Синхронизация здесь — та же, что уже стоит в цикле резкости,
            # просто добавленная туда, где её не было.
...
def resolve_queries(blocks, authored_queries=None):
    ...
    # ЗАПРОС СЛОТА = ЕГО СОБСТВЕННЫЙ БРИФ, а не запрос секции
    # (BRIEF_PRIMARY_QUERY=0/1, дефолт 1, 18.09).
    #
    # Найдено разметкой ГОТОВОГО отбора кофейного эпизода глазами, а не
    # чтением: 11 из 14 браков — это слот, у которого СВОЙ бриф написан
    # верно, а судил его запрос СОСЕДНЕЙ фразы той же секции:
    #
    #   фраза «римские священники требовали, чтобы Папа...»
    #     бриф автора:  a papal seal on an old document, close up
    #     запрос слота: vienna coffeehouse rain      -> дворец Хофбург
    #   фраза «то прямо запрещали, опасаясь...»
    #     бриф автора:  a rainy vienna street with an old coffeehouse sign
    #     запрос слота: ottoman sultan throne illustration -> улица с тук-туком
    #
    # Механизм не в модели и не в пороге. Авторских запросов на секцию
    # 4, а фраз 8, и semantic_query_assignment() обязан раздать те же
    # четыре строки восьми фразам — для половины из них подходящей строки
    # в списке НЕТ ФИЗИЧЕСКИ. Дальше эта строка становится ГЛАВНЫМ
    # запросом слота: по ней собирается пул, по ней судит гейт
    # релевантности, по ней считается корзина ранжирования и ключ кэша.
    # Бриф при этом добавлял кандидатов в пул (BRIEF_STOCK_QUERY), но
    # победителя выбирали по чужому вопросу — то есть система честно
    # приносила лучший ответ не на тот вопрос.
    #
    # Правка меняет ровно приоритет: есть свой бриф -> главный запрос
    # слота выводится ИЗ НЕГО (brief_to_stock_query — та же функция и тот
    # же короткий вид, что уже уходит в стоки, ≤5 слов плюс якорь эпохи,
    # поэтому распределение длин запросов не меняется и калибровка
    # CLIP_RELEVANCE_THRESHOLD/RISKY_QUERY_MARGIN остаётся в силе).
    # Запрос секции никуда не девается: он по-прежнему входит в пул слота
    # (section_query_pool в main()), просто перестаёт быть судьёй.
    #
    # Нет брифа, перевод пустой или не латиница -> прежнее поведение
    # БАЙТ-В-БАЙТ. Правка односторонняя: она может только заменить чужой
    # вопрос на собственный вопрос этого же слота.
    if feature_flags.enabled("BRIEF_PRIMARY_QUERY"):
        for i, b in enumerate(blocks):
            brief = (b.get("shot_brief") or "").strip()
            if not brief:
                continue
            bq = brief_to_stock_query(brief, fallback=None)
            if bq:
                raw[i] = bq
...
def candidate_gate_signature():
    ...
    try:
        ...
        parts = [inspect.getsource(f) for f in (
            ...
            # has_action_word/has_motion_word — решают, КАКАЯ функция вообще
            # вызывается первой (pexels_video vs pexels_photo, см. want_video
            # в main()), то есть из какого ПУЛА источников слот выбирает —
            # ровно "правила отбора кандидата", просто на уровень выше самого
            # is_relevant_candidate(). Реальный, найденный при добавлении
            # MOTION_STEMS (18.09) пробел: эта пара функций НИКОГДА не была
            # в этой подписи — ни при первом введении has_action_word, ни
            # сейчас — то есть на прогретом temp_smart/ смена набора
            # действие-слов/движение-слов ни разу не доходила до экрана,
            # клип брался готовым по старому решению фото/видео. Закрыто
            # заодно для обеих функций, а не только для новой.
            has_action_word, has_motion_word, action_video_qualifier,
        )]
        parts.append(repr((
            ...
            # Сами основы/таблицы-уточнители за has_action_word/has_motion_
            # word/action_video_qualifier выше — константы, а не код функции,
            # и inspect.getsource() их не видит. ACTION_STEM_EXCLUDE —
            # frozenset: сортировка ОБЯЗАТЕЛЬНА перед repr() — порядок его
            # repr() зависит от хэш-рандомизации Python и меняется МЕЖДУ
            # процессами при одном и том же содержимом (проверено: три
            # запуска подряд дали три разных строки). Без sorted() подпись
            # была бы недетерминированной не только при правке кода, а на
            # КАЖДОМ запуске пайплайна — весь кэш кандидатов инвалидировался
            # бы зря каждый раз. Поймано тестом test_selection_stack_
            # signature.py, а не рассуждением.
            ACTION_STEMS, tuple(sorted(ACTION_STEM_EXCLUDE)),
            ACTION_STEM_EXCLUDE_PREFIXES,
            MOTION_STEMS, ACTION_VIDEO_QUALIFIERS, MOTION_VIDEO_QUALIFIERS,
...
def pexels_video(query, index, used_ids=None, used_hashes=None, action_qualifier=None,
    ...
    try:
        ...
        for pq in pool_queries:
            ...
            # ЧЕРЕДОВАНИЕ ПО ИСТОЧНИКАМ, а не "весь Pexels, потом весь
            # Pixabay" — реальный, живьём воспроизведённый дефект (найден
            # 17.09 при проверке "не должен смотреть в одном источнике
            # больше, в другом меньше"): у фото ЭТУ ЖЕ болезнь нашли и
            # починили 13.09 (POOL_SOURCE_INTERLEAVE_VERSION, комментарий
            # выше в pexels_photo()), а на видео-путь чередование
            # ПО ИСТОЧНИКАМ (в отличие от чередования ПО ЗАПРОСАМ ниже) не
            # перенесли никогда. Цена — не гипотетическая: try_budget
            # (VIDEO_RELEVANCE_MAX_TRIES=3) реально СКАЧИВАЕТ кандидатов из
            # `ordered` ПОДРЯД с начала списка; для слота с ОДНИМ запросом
            # (самый частый случай — без extra_queries) список был
            # "все Pexels, потом все Pixabay", и живой прогон (замоканные
            # источники, 40+40 кандидатов) показал: первые 3 "offered" —
            # ВСЕ Pexels, Pixabay не скачивается НИ РАЗУ. Тот самый второй
            # видео-корпус, заведённый специально против брака 63% у видео
            # (см. её же комментарий ниже), для однозапросных слотов не
            # участвовал в реальном отборе вообще — только пополнял
            # SOURCE_STATS "offered", создавая иллюзию, что источник
            # работает. Тот же приём, что и у фото: обе выдачи собираются
            # раздельно и сплетаются zip_longest ДО того, как список уйдёт
            # в межзапросное чередование ниже.
            #
            ...
            for v in filter_alt_blocklist(_pixabay_search_videos(api_q)):
                v = dict(v)
                v["_origin_query"] = pq
                pixabay_list.append(v)
            lst = []
            for row in itertools.zip_longest(pexels_list, pixabay_list):
                for v in row:
                    if v is not None:
                        lst.append(v)
            per_query.append(lst)
...
def main():
    ...
    # (candidate_brief_keys) и его инвалидация уже умеют работать с этим
    ...
    for i, (b, d) in enumerate(zip(blocks, durs)):
        ...
        if (os.path.exists(out) and not feature_flags.enabled("SELECTION_ONLY")
                and not verify_clip(out, d)[0]):
            ...
            try:
                ...
                pass
        if os.path.exists(out) and not feature_flags.enabled("SELECTION_ONLY"):
            # В режиме SELECTION_ONLY кэш-хит КЛИПА пропускается сознательно:
            # клип всё равно не рендерится, а `continue` ниже уводит слот
            # МИМО подбора — то есть шотлист показал бы выбор ПРОШЛОГО
            # прогона вместо текущего кода, ровно то, ради чего режим и
            # заведён. Найдено первым же живым прогоном, а не чтением: все
            # 50 слотов кофейного эпизода вернулись `cache_hit` за 8 секунд,
            # и контактный лист описывал старые правила отбора. Кэш
            # КАНДИДАТОВ (pexels_cache, ключ с подписью гейтов) при этом
            # работает как обычно — он инвалидируется сам при смене правил.
            #
        ...
        if not photo and not video:
            ...
            # когда текст блока реально описывает действие/движение — БОЙ
            # (ACTION_WORDS — "штурм"/"погоня"/"удар") ИЛИ любое другое
            # физическое движение (MOTION_STEMS — "летит"/"раскрывается"/
            # "зависает"/"опускается", нишево-независимо; до этой правки
            # такая фраза падала на голый хэш h_text%2 — ровно то, против
            # чего заведён has_action_word, только для боя, не для движения
            # вообще, см. её докстринг). Хэш остаётся ТОЛЬКО базой для
            # визуального разнообразия там, где ни то ни другое не сработало
            # (иначе почти весь ролик без экшн/движения ушёл бы в чистое
            # фото).
            ...
            want_video = (has_action_word(b["text"]) or has_motion_word(b["text"])
                         or h_text % 2 == 1) and not stat
        ...
        if (video and not photo and not locked_shot
            ...
            #
            # РЕАЛЬНЫЙ, живым прогоном найденный краш (19.09, чистый прогон
            # после фикса FRAME_VERIFIER_GAVE_UP): _slot_miss_snapshot()
            # была написана и задокументирована как вызываемая ЗДЕСЬ, но
            # реальный вызов `snapshot = _slot_miss_snapshot(i)` никогда не
            # был добавлен — переменная `snapshot` использовалась ниже
            # (строки reason/_slot_miss_restore) НЕОПРЕДЕЛЁННОЙ. Седьмой
            # случай класса «слой написан, задокументирован — и не вызван»,
            # и первый из них, что не молчит fail-open, а роняет весь
            # рендер NameError на любом слоте, где VIDEO_PHOTO_RESCUE вообще
            # срабатывает.
        ...
        # РЕЖИМ «ТОЛЬКО ОТБОР» (SELECTION_ONLY=1): прогон доходит до конца
        # отбора, пишет шотлист и ВСЕ отчёты, и не рендерит ни одного клипа.
        #
        # Зачем отдельный режим, а не «посмотреть готовый ролик»: между
        # «слот подобран» и «видно, что подобрано» стоял полный рендер —
        # десятки минут ffmpeg на эпизод, при том что решение об отборе
        # принято задолго до первого кадра. Для проверки «какие кадры
        # система выбирает сейчас» рендер не нужен вообще: контактный лист
        # строится из шотлиста (scripts/shotlist_contact.py), а он готов
        # уже здесь.
        #
        # Слот при этом проходит ВЕСЬ настоящий путь отбора — те же гейты,
        # тот же пул, те же отчёты: это не облегчённая копия, а тот же код
        # без последнего шага. Поэтому контактный лист такого прогона
        # показывает ровно те кадры, что встали бы в ролик.
        if feature_flags.enabled("SELECTION_ONLY"):
            render_manifest[i] = {"index": i, "status": "skipped_selection_only",
                                  "reason": "SELECTION_ONLY=1 — клип не рендерился"}
            continue
        future = None
        try:
            if video:
    ...
    if feature_flags.enabled("SELECTION_ONLY"):
        # Выход ДО ожидания футур и сборки: рендеров не было, ждать нечего.
        # Отчёты отбора (шотлист, вклад источников, отчёт о слоях, промахи
        # релевантности) пишутся НИЖЕ по коду — поэтому здесь не return, а
        # флаг, который гасит только рендерную часть.
        pending_jobs = []
    ...
    if feature_flags.enabled("SELECTION_ONLY"):
        # Все отчёты отбора уже записаны выше. Сборка не запускается, и
        # жёсткий гейт ниже здесь не при чём: клипов не было не из-за сбоя,
        # а по прямому заказу. Молча уйти с кодом 1 значило бы назвать
        # штатный режим отказом.
        print(f"\nSELECTION_ONLY: отбор завершён, рендер не запускался. "
              f"Слотов: {len(blocks)}. Контактный лист: "
              f"python scripts/shotlist_contact.py {VIDEO_FOLDER}")
        return EXIT_OK
