...
def film_look(photo_hash, section="", brightness_bias=0.0, energy_bias=0.0, levels=None, wb=None):
    ...
    # Тёплый источник -> не давим фирменный тёплый пуш ещё сильнее (иначе
    # "апельсиновая каша"); холодный источник -> пуш можно чуть усилить.
    warm_mult = 1.0 - 0.35 * max(0.0, warm_bias) + 0.20 * max(0.0, -warm_bias)
