...
#
# ОНО БОЛЬШЕ НЕ ДЕФОЛТ КОДА (17.09, измерено). Раньше 900-1600 подставлялось
# любому каналу и любому эпизоду, причём channel_profile.json ЭТОГО канала
# era_from/era_to не объявлял вообще — то есть медиевализм жил не в профиле
# ниши, а в константе модуля. Прямой замер на пустом профиле:
#     era_overlaps(1940, 1945) -> False   (предмет Второй мировой)
#     era_overlaps(-8000, -6000) -> False (кремнёвый топор)
# Для НЕ средневековой исторической темы это худший из возможных исходов:
# авто-ниша честно говорит «музеи этой теме подходят» (для каменного века
# или Рима это правда — музей там лучший источник), музеи спрашиваются, и
# КАЖДЫЙ правильный предмет молча выбрасывается паспортом по окну, которое
# никто не объявлял. Ни одной строки в логе, ноль кандидатов, причина
# неотличима от «в музее этого нет».
#
# Теперь окно берётся ТОЛЬКО из профиля (канал объявляет его сам, см.
# channel_profile.json, либо авто-ниша эпизода — content_world.py). Никто не
# объявил — окна НЕТ, и музейный путь для этого эпизода не используется
# вовсе (см. search_museums): угадать эпоху нельзя, а выдавать угаданное
# окно за паспортную проверку — это ложная гарантия. Слот при этом не
# пустеет: музеи аддитивны, стоки и архивы работают как раньше.
#
# Константы остаются ТОЛЬКО как значение, которое ставит себе канал, не
# объявивший окно явно в старом профиле (обратная совместимость офлайн-
# сборщиков индекса), и как документация происхождения числа.
...
def _warn_once(key, message):
    """Одна строка на прогон: причина обязана быть названа, но не сотни раз
    (search_museums зовётся на каждый запрос каждого слота)."""
    if key in _WARNED:
        return
    _WARNED.add(key)
    print(message)
...
def era_window_declared():
    """Окно эпохи, ОБЪЯВЛЕННОЕ каналом или авто-нишей эпизода, иначе None.

    Отличать «объявлено» от «подставлено дефолтом» обязательно: паспортная
    проверка имеет смысл ровно тогда, когда эпоха известна. Не объявлено —
    значит проверять нечем, и это должно быть видно вызывающему, а не
    спрятано за медиевализмом из константы.
    """
    p = _profile()
    lo, hi = p.get("era_from"), p.get("era_to")
...
def era_window():
    """Окно для паспортной проверки. Не объявлено — прежние константы, но
    ЕДИНСТВЕННЫЙ живой путь (search_museums) до этого места уже не доходит:
    он останавливается на era_window_declared() is None. Фолбэк оставлен для
    офлайн-сборщиков индекса (met_catalog/europeana_corpus), которые человек
    запускает руками и которые предупреждают об этом явно."""
    w = era_window_declared()
    if w is not None:
        return w
...
def search_museums(query, department=None, limit=None):
    ...
    if _profile().get("use_museum_sources", True) is False:
        return []
    # ЭПОХА НЕ ОБЪЯВЛЕНА — МУЗЕЙ НЕ СПРАШИВАЕТСЯ. Весь смысл этого модуля в
    # том, что эпоха предмета ИЗВЕСТНА ИЗ ПАСПОРТА, а не угадана по
    # пикселям (см. докстринг модуля). Паспорт сверяется с окном эпохи —
    # и если окно не объявил ни канал, ни авто-ниша эпизода, сверять не с
    # чем. Раньше сюда подставлялось 900-1600 из константы модуля, то есть
    # СРЕДНЕВЕКОВЬЕ ЛЮБОЙ ТЕМЕ: замер на пустом профиле показал, что предмет
    # 1940-1945 и кремнёвый топор -8000..-6000 отбраковываются паспортом
    # молча, без единой строки в логе, и причина неотличима от «в музее
    # этого нет». Для не средневековой исторической темы это худший исход:
    # авто-ниша честно включает музеи (для каменного века или Рима это
    # верно), а окно из константы выбрасывает КАЖДЫЙ правильный предмет.
    # Отказ здесь ничего не ломает: музеи аддитивны, слот собирается из
    # стоков и архивов, как до подключения этого модуля.
    if era_window_declared() is None:
        _warn_once("era_window_undeclared",
                   "  МУЗЕИ ПРОПУЩЕНЫ: окно эпохи не объявлено ни каналом "
